...
public class JWSTokenProvider implements Token.Provider<JWSToken> {
    ...
    public JWSToken issue(Account account) {
        ...
        builder.id(UUID.randomUUID().toString()).rsa256(getPrivateKey())
                .issuer(account.getPartition().getName())
                .issuedAt(getCurrentTime()).subject(account.getId())
        ...
        try {
            isNewTransaction = Status.STATUS_ACTIVE != this.userTransaction
                    .getStatus();
            ...
            throw new AuthenticationException(
                    "Could not issue token for account [" + account + "]", e);
        }
    ...
    public void invalidate(Account account) {
        getIdentityManager().removeCredential(account,
